...
class Barracks_Select(discord.ui.Select):
    def __init__(self, author, unit, building, last):
        self.author = author
        self.unit = unit
...
class Barracks(discord.ui.View):
    def __init__(self, author, unit, building, *, timeout: Optional[float] = 40):
        ...
        post = info_collection.find_one({"_id": self.unit["race"]})
        if not post:
            last = 0
        else:
            last = post["unlocked"]
        ...
        self.add_item(Barracks_Select(self.author, self.unit, self.building, last))
