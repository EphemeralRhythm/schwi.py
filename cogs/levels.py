...
from database import xp_collection as profiles
...
def get_post(id):
    ...
    if not player_post:
        ...
        post = profiles.insert_one(player_post)
    return player_post
...
class Levels(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not message.author.bot:
            ...
            diff = int(time.time() - player_post.get("timeout"))
            if diff < 60:
                return
            ...
            result = profiles.update_one(filter, update)
    ...
    async def rank(self, ctx, member : typing.Optional[discord.Member] = None ):
        ...
        xpcard = f"images/xpcards/zIMAGE.png"
        ...
        background.blend(image=ima, alpha=.6, on_top=False)

        background.paste(profile.image, (30, 30))

        background.rectangle((30, 220), width=780, height=40, fill="#fff", radius=20)
        if percentage > 2: background.bar(
            (30, 220),
            max_width=780,
            height=40,
    ...
    async def top(self, ctx, arg : typing.Optional[int] = 1 ):
        arg -= 1
        if arg < 1:
            arg = 0
        ...
        background = Editor(f"images/xpcards/top.png").resize((650,650))
        ...
        for i in range(iterations):
            ...
            editor.rectangle((0, i * box_height + 14 * i), width=box_width, height=box_height, fill="#000", radius= 20)
            editor.text(
        ...
        embed = discord.Embed(title = "No Chess No Life All Time Rankings", color=discord.Color.from_rgb(201,0,118))
...
You are rank `#{rank_index}` on this server\nwith a total of `{player_post.get('xp')}` <:chat_bubble:1076148573718196294>"""
        ...
        if arg * 10 > len(players):
            embed.set_image(url = "https://media.discordapp.net/attachments/1073538985529446511/1076152373736640673/top.png")
        else:
            url = message.attachments[0].url
            embed.set_image(url = url)
            embed.set_footer(text= f"Page {arg + 1} • Type .top  {arg + 2} to go to page {arg + 2} of the leaderboard")
